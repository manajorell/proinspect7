...
object PdfGenerator {
    ...
    private fun addHousePhotoPage(doc: Document, photos: List<InspectionPhoto>) {
        ...
        tc.paddingLeft = 14f
        tc.paddingRight = 14f
        tc.addElement(Paragraph("Property Photo", fWhite))
        tc.addElement(Paragraph("Front exterior view of inspected property", fWhiteSm))
        titleTbl.addCell(tc)
        doc.add(titleTbl)
        ...
        try {
            ...
            if (photoFile.exists()) {
                ...
                if (bmp != null) {
                    ...
                    imgCell.border = Rectangle.BOX
                    imgCell.borderColor = cBorder
                    imgCell.borderWidth = 2f
                    imgCell.paddingTop = 10f
                    imgCell.paddingBottom = 10f
                    imgCell.paddingLeft = 10f
                    imgCell.paddingRight = 10f
            ...
            noteCell.backgroundColor = BaseColor(254, 243, 199) // Light yellow
            noteCell.border = Rectangle.BOX
            noteCell.borderColor = cOrange
            noteCell.paddingTop = 12f
            noteCell.paddingBottom = 12f
            noteCell.paddingLeft = 12f
            noteCell.paddingRight = 12f
            noteCell.addElement(Paragraph("House photo unavailable",
                Font(Font.FontFamily.HELVETICA, 10f, Font.ITALIC, cGray)))
    ...
    private fun addSignedAgreement(doc: Document, report: Report, context: Context) {
        // Check if signed agreement path exists
        ...
        try {
            ...
            hdrCell.backgroundColor = cNavy
            hdrCell.border = Rectangle.NO_BORDER
            hdrCell.paddingTop = 10f
            hdrCell.paddingBottom = 10f
            hdrCell.paddingLeft = 14f
            hdrCell.paddingRight = 14f
            ...
            if (bmp != null) {
                ...
                // Add caption
                val caption = Paragraph("Inspection agreement signed by client on ${report.inspectionDate}",
